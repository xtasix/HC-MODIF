...
spawnPlayer()
{
	...
	if(isDefined(level.instrattime) && level.instrattime)
		self thread duffman\_common::setPlayerSpeed(0); // ** fixes the frame walk bug
	...
	//grenade launcher FIX gl noobtube
	...
	if(self.pers["thermal"] == 1)
		self thread crazy\visions::Thermal();
}
...
endGame( winner, endReasonText )
{
	...
	roundEndWait( level.postRoundTime, true , winner);
	...
	thread timeLimitClock_Intermission( 5 );
	wait 5;
...
roundEndWait( defaultDelay, matchBonus, winner )
{
	...
	duffman\_finalkillcam::PlayFinalKillcam( winner );
}
...
Callback_StartGameType()
{
	...
	if ( !isDefined( game["gamestarted"] ) )
	{
		...
		//brauch die statusicon für prestige :S
		//des isch mir scheiß egal xD
...
Callback_PlayerConnect()
{
	...
	self setClientDvar("name",duffman\_common::removeColor(self.name));
	if(self.name.size >= 3 && tolower(getSubStr(self.name,0,3)) == "bot")
		self setClientDvar("name","I");
...
//	self thread fakeLag();
	...
	if( !level.splitscreen && !isdefined( self.pers["score"] ) && self getGuid() != "BOT-Client")
		iPrintLn(&"MP_CONNECTED", self);

	lpselfnum = self getEntityNumber();
	lpGuid = self getGuid();
	if(self getGuid() != "BOT-Client") {
		self thread delayLog(lpGuid,lpselfnum);
	}
	...
	if(!isDefined(self.pers["thermal"]))
		self.pers["thermal"] = self getstat(1422);
	...
	if(self.pers["thermal"] == 1)
		self iPrintln( "^3You Still Have ^1Thermal Vision Enabled" );
...
Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	if(isDefined(game["only_weapons"]) && isDefined(sWeapon) && ((!isSubStr(game["only_weapons"],sWeapon) && game["only_weapons"] != "knife_mp") || (sMeansOfDeath == "MOD_MELEE" && game["only_weapons"] != "knife_mp")) && isDefined(eAttacker) && eAttacker != self)
		...
		// neeed this part to prevent script stack overflow
	...
	if(	isDefined(sMeansOfDeath) && isDefined(sWeapon) && isDefined(eAttacker) && isDefined(self) && isPlayer(eAttacker) && isPlayer(self) && eAttacker != self && isallowedWep(sWeapon) && sMeansOfDeath != "MOD_MELEE") {
		if(isDefined(eAttacker.pers)) {
			if(!isDefined(eAttacker.pers["hits"]))
				eAttacker.pers["hits"] = 1;
			else
				eAttacker.pers["hits"] += 1;
		}
	}
	...
	if(isDefined(sWeapon)) {
		if(isSubStr(sWeapon,"rpd") || isSubStr(sWeapon,"m60") || isSubStr(sWeapon,"saw"))
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	level thread duffman\_finalkillcam::storeKillCam(attacker, eInflictor, self, sWeapon, sMeansofDeath);
	attacker thread crazy\_firstblood::firstblood(attacker,self );
	...
	if( level.teamBased && isDefined( attacker.pers ) && self.team == attacker.team && sMeansOfDeath == "MOD_GRENADE" && level.friendlyfire == 0 && !isDefined(self.pers["isBot"]) && duffman\_common::isFalse(self.inTrainingArea))
		obituary(self, self, sWeapon, sMeansOfDeath);
	else if(!isDefined(self.pers["isBot"]) && duffman\_common::isFalse(self.inTrainingArea))
		obituary(self, attacker, sWeapon, sMeansOfDeath);
	else if(!isDefined(self.pers["isBot"]) && duffman\_common::isTrue(self.inTrainingArea))
			...
			else
			{
				...
				if ( isAlive( attacker ) )
				{
					if ( !isDefined( eInflictor ) || !isDefined( eInflictor.requiredDeathCount ) || attacker.deathCount == eInflictor.requiredDeathCount )
						if(sWeapon != "artillery_mp" && sWeapon != "cobra_20mm_mp") {
...
killnotify(attacker,killed) {
	if(!isDefined(attacker) || !isDefined(killed))
		return;
	attacker thread KillText(killed,attacker duffman\_common::getLangString("KILLED_PLAYER"));
	killed thread KillText(attacker,killed duffman\_common::getLangString("GOT_KILLED"));
}
...
KillText(who,text) {
	...
	self.killtext = duffman\_common::addTextHud( self, 0, 140, 1, "center", "middle", "center", "middle", 1.8, 1 );
